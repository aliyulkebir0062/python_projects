...
TOKEN = "7063926293:AAFsoyyNw8p0n8_NXNaXP73aNjqFyb5Q4mc"  
...
async def video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Video kaydı başlatılıyor...")
    record_video()
    ...
    await update.message.reply_text("Video gönderildi.")

async def foto(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("📸 Fotoğraf çekiliyor...")
    take_photo()
    chat_id = update.effective_chat.id
    with open("kamera_fotografi.jpg", "rb") as photo:
        await context.bot.send_photo(chat_id=chat_id, photo=photo)
    await update.message.reply_text("Fotoğraf gönderildi.")
